...
let sortTable = (idTable, data) => {         //<--- ГЛАВНАЯ ФУНКЦИЯ СОРТИРОВКИ
 ...
 /*   let compareG = (a,b) => {
    ...
    let compareS = (a,b) => {
      return (a['Продано (млн.)'] <= b['Продано (млн.)']) ? -1 : 1
    }
    ...
    /*  let compareMulti = (a, b) => {
        // 1-й уровень: по поколению
        if (a['Поколение'] !== b['Поколение']) {
          return a['Поколение'] <= b['Поколение'];
        }
        // 2-й уровень: по продажам (если поколение одинаковое)
        return a['Продано (млн.)'] <= b['Продано (млн.)'];
      }
